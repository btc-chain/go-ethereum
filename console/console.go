...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"btc": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + btc.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + btc.blockNumber + " (" + new Date(1000 * btc.getBlock(btc.blockNumber).timestamp) + ")\n";
		try {
